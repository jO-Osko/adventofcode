...
def part1(data, reference):
    references = {}
    for ref in reference.split("\n"):
        ref = ref.strip().split(":")
        references[ref[0].strip()] = int(ref[1])
    return solve(data.strip().split("\n"), references)

def part2(data, reference):
    references = {}
    for ref in reference.split("\n"):
        ref = ref.strip().split(":")
        references[ref[0].strip()] = int(ref[1])
...
def solve(data, references, broken=False):
    for line in data:
        name, numbers = line.strip().split(":",1)
        ...
        for k in numbers:
            ...
            if broken:
                if na in ("cats", "trees"):
                    if references[na] >= int(nu):
                        break
                elif na in ("pomeranians", "goldfish"):
                    if references[na] <= int(nu):
                        break
                elif references[na] != int(nu):
                    break   
            else:
                if references[na] != int(nu):
                    break        
...
DATA = """Sue 1: cars: 9, akitas: 3, goldfish: 0
...
print(part1(DATA, REFERENCES))

print(part2(DATA, REFERENCES))
